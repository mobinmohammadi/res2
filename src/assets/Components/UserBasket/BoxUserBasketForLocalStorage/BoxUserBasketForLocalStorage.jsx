...
export default function BoxUserBasketForLocalStorage({ foods ,localStorageData,
 CalculatorUserBasket,addToBasketUser , item ,menu , setIdProductInBasket , deleteFoodInUserBasket }) {
  ...
  const getProductCount = (id) => {
    const item = localStorageData.find((p) => p.id == id);
  ...
  return (
    ...
    >
      <div key={foods.id} className="flex justify-between w-full gap-2">
        <img
        ...
        <div className="flex justify-around flex-col">
          ...
          <div className="text-xs flex gap-1">
            <span>{foods.price.toLocaleString()}</span>
            <span>تومان</span>
          </div>
        </div>
        <div className="flex flex-col items-center justify-around ">
          <div className="flex items-center gap-1">
            ...
            <span>{getProductCount(foods.id)}</span>
            <svg
              // ref={svgUserBasket}
          ...
          >
            ...
            <button
              ref={btnDeleteFood}
              onClick={() => handleDeleteInLocalStorage()}
